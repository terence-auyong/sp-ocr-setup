...
export const resolvePayloads = (
    ...
    const extModulesMap = new Map(
        extModules.map((m) => [m.name.toLowerCase(), m]),
    );
    ...
    const storeMap = new Map(
        allStores
            .filter((s) => s?.store_code != null)
            .map((s) => [s.store_code.toLowerCase(), s]),
    );
    ...
    const storeGroupMap = new Map(
        allStoreGroups
            ...
            .map((g) => [g.code.toLowerCase(), g]),
    );
    const storeTypeMap = new Map(
        allStoreTypes
            .filter((t) => t?.code != null)
            .map((t) => [t.code.toLowerCase(), t]),
    );

    const addErrorToRows = (
        rowNums: number[],
        msg: string,
        column: string = '',
    ) => {
    ...
    for (const raw of raws) {
        ...
        for (const batch of raw.batches) {
            if (!Number.isFinite(batch.maxScan) || batch.maxScan <= 0) {
                const msg = `Max Scan must be a positive number`;
                errors.push(`${prefix}: ${msg}`);
                addErrorToRows(batch.rowNumbers, msg, '');
            }
        }

        // 5. OCR API Resolution
        let apiErrorMsg = '';
        const ocrApiNameLower = raw.ocrApiName
            ? raw.ocrApiName.toLowerCase()
            : '';
        ...
        const resolvedBatches: Batch[] = raw.batches.map((batch) => {
            const resolvedGroups: Group[] = batch.groups.map((rawGroup) => {
                ...
                const moduleFlags = [
                    {
                        name: 'Inventory',
                        val: rawGroup.moduleInventory,
                        column: 'inventory',
                    },
                    {
                        name: 'Near Expiry',
                        val: rawGroup.moduleNearExpiry,
                        column: 'near expiry',
                    },
                    { name: 'OSA', val: rawGroup.moduleOsa, column: 'osa' },
                    {
                        name: 'Share of Shelf',
                        val: rawGroup.moduleShareOfShelf,
                        column: 'share of shelf',
                    },
                ];
                ...
                moduleFlags.forEach((flag) => {
                    if (!isValidYN(flag.val)) {
                        const msg = `Missing required value (${flag.name}).`;
                ...
                if (!rowHasFormatError) {
                    if (raw.ocrCode === 'OCR Inside INV') {
                        const invalidModules = moduleFlags.filter(
                            (m) => m.name !== 'Inventory' && isTrue(m.val),
                        );
                        ...
                        if (
                            isValidYN(rawGroup.moduleInventory) &&
                            !isTrue(rawGroup.moduleInventory)
                        ) {
                            const msg =
                                'Inventory is required for OCR Inside INV.';
                    ...
                    if (raw.ocrCode === 'Multiple OCR Module') {
                        const selected = moduleFlags.filter((m) =>
                            isTrue(m.val),
                        );
                        if (selected.length === 0) {
                            const msg =
                                "At least one module must be 'Y' for Multiple OCR Module.";
                ...
                const getIDZero = <T extends { id: number }>(
                    map: Map<string, T>,
                ): T => {
                    return (
                ...
                if (channelCode) {
                    const found = storeChannelMap.get(
                    ...
                    if (!found) {
                        ...
                        if (
                            siteGroup.id > 0 &&
                            siteGroup.store_channel_id !== found.id
                        ) {
                ...
                if (storeGroupCode) {
                    const found = storeGroupMap.get(
                        storeGroupCode.toLowerCase(),
                    );
                    if (!found) {
                ...
                if (storeCode) {
                    ...
                    if (!found) {
                        ...
                        if (
                            storeType.id > 0 &&
                            store.store_type_id !== storeType.id
                        ) {
                        ...
                        if (
                            storeGroup.id > 0 &&
                            store.store_group_id !== storeGroup.id
                        ) {
                ...
                if (
                    ...
                    const msg = 'Start Date must not be later than End Date.';
                    errors.push(`${rawGroup.rowNumber}: ${msg}`);
                    addErrorToRows(currentRow, msg, 'start date');
                    addErrorToRows(currentRow, msg, 'end date');
                }
        ...
        const moduleFlags = [
            ...
            {
                name: 'Share of Shelf',
                isSelected: isTrue(raw.moduleShareOfShelf),
            },
        ];

        moduleFlags.forEach((flag) => {
            if (flag.isSelected) {
                const found = extModules.find(
                    (m) => m.name.toLowerCase() === flag.name.toLowerCase(),
                );
                if (found) {
        ...
        const {
            ...
            ocrApiName: _an,
            moduleInventory: _mi,
            moduleNearExpiry: _mne,
            moduleOsa: _mo,
            moduleShareOfShelf: _ms,
            batches: _b,
            rowNumbers: _rn,
            emptyLocationRows: _el,
            ...rest
        } = raw as any;
